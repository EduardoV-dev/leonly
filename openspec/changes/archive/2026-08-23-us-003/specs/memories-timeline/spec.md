...
preview or fallback, and any available description and location previews. Each card MUST provide
stable extension regions for later memory-count, detail-link, and action capabilities without
requiring the timeline summary contract to change.
...
- **WHEN** a later capability contributes a count, detail link, or action to a card
- **THEN** that content is rendered in its designated extension region while the timeline summary
   remains available
