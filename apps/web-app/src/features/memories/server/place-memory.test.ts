...
describe("placeMemory", () => {
  ...
  it.each([
    ...
    async (_source, target) => {
      ...
      const result = await placeMemory(MEMORY_ID, target, EXPECTED_VERSION);
  ...
  it.each([
    ...
    async (outcome, status, code) => {
      ...
      await expect(placeMemory(MEMORY_ID, "vault", EXPECTED_VERSION)).rejects.toMatchObject({
        code,
        status,
      });
    },
  ...
  it("rejects malformed versions without calling the database", async () => {
    await expect(placeMemory(MEMORY_ID, "vault", "not-a-version")).rejects.toThrow(
