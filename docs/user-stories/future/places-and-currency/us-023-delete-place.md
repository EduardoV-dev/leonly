...
`is_active = false`. The place, expense items, and ratings become inaccessible through product
routes, and the cover becomes inaccessible through product and storage routes. Retained data remains
available only under the MVP retention policy.
...
- Retained expense items and ratings for a deleted place are excluded from every total, average, and
  ranking. Retained cover objects are denied through storage routes.
- If deletion fails, the place remains active and the UI shows retryable feedback without claiming success.
- Permanent place, expense-item, rating, and media cleanup and place restoration are outside the MVP.
...
- Retained expense items and ratings for the deleted place do not affect any displayed total, average,
  or ranking.
...
- Verify confirmation, cancellation, successful soft deletion, repeated deletion, retained-item and
  rating exclusion, and cover denial.
