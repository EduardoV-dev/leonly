...
Either active member can edit an active place's name, category, description, location, and cover. The
form starts from the latest readable values and does not modify expense items, ratings, or creator
ownership.
...
- Name, category, description, location, and cover use the same limits, normalization, enum, and file
  validation decided for US-017.
...
- Client-supplied space, creator, expense-item, or rating values are ignored or rejected.
...
- Invalid text, category, and cover input is rejected server-side without changing confirmed data.
...
- Payload tampering cannot change space, creator, expense items, ratings, or another space's place.
