...
Each active membership owns one preferred currency. This story establishes the preference,
authoritative timestamped exchange-rate snapshot, and conversion contract consumed by place views
without changing stored place amounts or currencies.

## Scope

- Personal USD or NIO preference selection, save, pending, success, failure, and refresh states.
- USD-to-NIO, NIO-to-USD, and same-currency conversion behavior for place consumers.
- Current-rate retrieval, timestamped cached fallback, no-rate fallback, and stable result-set
  snapshots.

## Business Rules

- Supported stored and preferred currencies are `USD` and `NIO`; all other submitted values are rejected.
- Application configuration exposes one canonical supported-currency list containing exactly `USD`
  and `NIO`. Preference controls, place item forms, conversion, and server validation use that list.
...
- Each membership chooses and stores its own default display currency; changing it never changes the
  partner's preference or any expense item's original currency.
...
- Provider failure uses the last successful rate. If no cached rate exists, consumers receive no
  converted value and keep original amounts and currencies usable.
...
- The conversion contract preserves each original amount and currency and returns a converted value
  only when one is available.
- Same-currency amounts are not converted, and every conversion in one result set uses the same
  current or cached rate snapshot.
- Provider failure uses the last successful timestamped rate, or returns no converted value when no
  rate exists, without blocking consumers.
...
- Client and server currency choices contain only the configured `USD` and `NIO` values.
...
- Define the initial selection flow for a new or existing membership with no saved preference.
...
- Test every page of one result set uses the same authoritative rate snapshot; US-019 verifies sorting
  and card presentation, and US-020 verifies detail presentation.
