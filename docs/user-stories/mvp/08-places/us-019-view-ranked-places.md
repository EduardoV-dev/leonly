...
The places page shows a bounded, cursor-paginated list of active places from the member's active
space. Each card shows the name, category, cover photo or fallback, average rating or unrated state,
expense total in the member's preferred currency when available, location, and description preview.
...
- Places without expense items or without an available single-currency total follow places with totals
  in both budget sorts. Budget ties use a deterministic ID order.
- Expense-total display and sorting use the same authoritative rate snapshot for the entire result
  set, as defined by US-016 and US-018. If no required rate is available, original item amounts remain
  available on detail and no converted total is used for sorting.
...
- Each supported sort returns deterministic pages, including ties and missing ratings or expense totals.
- Cards show defined fallbacks for absent photos, ratings, expense totals, locations, and descriptions.
- The conversion rate used to order expense totals is the rate represented in displayed values.
- Initial load and subsequent pages expose accessible loading and retryable error feedback.

## Verification Notes

- Verify every sort with ties, null ratings, empty expenses, unavailable totals, and mixed currencies.
