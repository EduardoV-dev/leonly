...
As an active member, I want every completed MVP feature's access controls verified together so no
unauthenticated, inactive, or cross-space user can observe or change private data.

## Intended Outcome

This final security audit verifies that every protected read and mutation derives identity from the
authenticated session and permits access only when that identity has an active membership in the
record's active space. Each owning story must already ship its database RLS, constraints, storage
policies, server boundaries, and focused access tests; this story closes cross-feature gaps.
