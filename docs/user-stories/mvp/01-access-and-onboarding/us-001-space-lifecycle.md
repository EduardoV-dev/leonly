# US-001: Enforce Space Lifecycle

**Priority:** Must<br>
**Depends on:** None<br>
**External prerequisite:** Google authentication
...
- The minimal authenticated dashboard route shell required as the successful routing destination.
- Safe handling of invalid, expired, used, regenerated, and concurrently redeemed invites.
- Server-side rate limiting for join-code redemption.
...
- Each authenticated user may submit at most five failed join-code redemption attempts in a rolling
  10-minute window. A sixth attempt starts a 10-minute lock and is rejected before code lookup with
  HTTP `429`, `Retry-After: 600`, and the message `Too many join attempts. Try again in 10 minutes.`
- Failed code validation or redemption counts toward the limit; transient server failures do not. A
  successful redemption or 10 minutes without a failed attempt clears the failed-attempt count.
  Requests during a lock do not extend it and return `Retry-After` as the remaining whole seconds.
- Rate-limit state is enforced atomically on the server and cannot be bypassed with concurrent requests.
...
- Creating an active space atomically creates the creator's active membership, completes onboarding,
  and routes to the one-member dashboard shell.
...
- The first five failed join-code attempts receive the applicable safe error. The sixth receives HTTP
  `429`, `Retry-After: 600`, and `Too many join attempts. Try again in 10 minutes.` without a code
  lookup or membership change; requests during the lock return its remaining duration.
- Post-login routing sends no-active-space users to create/join setup and active-space users to the
  dashboard shell with the correct one-member or two-member state.
...
- Define invite-code format, entropy, and normalization.
...
- Verify the five-attempt boundary, atomic concurrent attempts, `Retry-After: 600`, no lookup while
  blocked, successful-redemption reset, and reset after 10 minutes.
