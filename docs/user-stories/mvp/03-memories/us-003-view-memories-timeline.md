...
- Each memory card shows its title, date, cover-photo preview or fallback, and optional description
  and location previews. It provides extension regions for the counts, detail link, and actions added
  by later stories.
...
- Cards show the required summary fields, valid optional fields, and a photo fallback when no cover
  exists; later count, detail, and action stories integrate through the defined extension regions.
